...
import { Reveal, BigCTA, Footer, ProjectCard } from './components';
...
const ERAS = [
  {
    ...
    count_zh: "3 个项目",
    projects: [
      { slug: "luci-ai",     title: "LUCI AI",        year: "25 — now",       role_en: "AI PM",                 role_zh: "AI 产品经理",     img_en: "Personal Memory Companion",    img_zh: "个人记忆伴侣",   tone: "steel",     tags: ["Hardware × App", "Branding"], image: "images/work/luci-ai.png", position: "center top", cats: ["AI PM", "Brand"] },
      { slug: "memories-ai", title: "memories.ai",    year: "25.05 — 25.11",  role_en: "PM & Creative Director", role_zh: "产品经理 & 创意总监", img_en: "Large Visual-Memory Model · LVMM", img_zh: "大型视觉记忆模型 · LVMM", tone: "mono",      tags: ["LVMM", "Video Agents", "CIS"], image: "images/work/memories-ai.jpg", cats: ["AI PM", "Brand"] },
      { slug: "mincho-ai",   title: "Mincho",         titleEm: "AI · Beta",   year: "25.01 — 25.04",            role_en: "AI PM & UX",            role_zh: "AI 产品经理 & 体验设计", img_en: "Conversational mental healing", img_zh: "对话式心理疗愈", tone: "blue",      tags: ["Mini-Program", "Prompt UX"], image: "images/work/mincho-ai.png", position: "left center", cats: ["AI PM", "UX"] },
    ],
  },
  {
    years: "23 — 24",
    ...
    count_zh: "2 个项目",
    projects: [
      { slug: "emosync",      title: "EmoSync",      year: "2024", role_en: "MDes Final",   role_zh: "MDes 毕业作品",  img_en: "Embodied interaction · wearable", img_zh: "具身交互 · 可穿戴",     tone: "mono-blue",  tags: ["Wearable", "HCI"], image: "images/work/emosync.jpg", position: "left center", cats: ["Interaction", "UX"] },
      { slug: "nature-times", title: "Nature Times", year: "2024", role_en: "Speculative",  role_zh: "思辨设计",      img_en: "Citizen science newspaper",        img_zh: "公民科学报纸",          tone: "mono-mist",  tags: ["Editorial", "N4H"], image: "images/work/nature-times.jpg", cats: ["Interaction"] },
    ],
  },
  {
    years: "21 — 23",
    ...
    cats: ["Industrial", "Interaction"],
    searchKeys: "easelbow mind box performance city stage digital imaging industrial speculative interactive media storytelling 工业 思辨 交互 叙事 before ai 21 23",
  },
];

/* category + keyword matcher */
function projectMatches(p, tag, q) {
  const tagOK = tag === "All" || (p.cats && p.cats.includes(tag));
  if (!tagOK) return false;
  if (!q) return true;
  const hay = [p.title, p.titleEm, p.role_en, p.role_zh, p.img_en, p.img_zh, ...(p.tags || [])].join(" ").toLowerCase();
  return hay.includes(q);
}
function archiveMatches(era, tag, q) {
  const tagOK = tag === "All" || (era.cats && era.cats.includes(tag));
  if (!tagOK) return false;
  if (!q) return true;
  const hay = [era.title_en, era.title_zh, era.searchKeys, ...(era.tags || [])].join(" ").toLowerCase();
  return hay.includes(q);
}
...
function WorkPage({ go, goProject }) {
  ...
  // compute, per era, which projects survive the filter + search
  const filteredEras = ERAS.map(era => {
    if (era.archive) {
      return { era, visible: archiveMatches(era, filter, q) ? "archive" : null };
    }
    const projects = era.projects.filter(p => projectMatches(p, filter, q));
    return { era, visible: projects.length ? projects : null, projects };
  });
  const anyVisible = filteredEras.some(e => e.visible);
  ...
  return (
    <main className="page-root" data-screen-label="Work">
        ...
        <Reveal delay={320}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 96, paddingTop: 24, borderTop: "1px solid var(--line)", gap: 24, flexWrap: "wrap" }}>
            ...
            <div className="mono-cap" style={{ display: "inline-flex", alignItems: "center", gap: 12 }}>
              ...
              <input className="input-field" value={query} onChange={e => setQuery(e.target.value)} placeholder={t("e.g. agent, brand, wearable", "如：智能体、品牌、可穿戴")} style={{ width: 240, fontSize: 13, padding: "8px 0", borderBottomWidth: 1 }} />
            </div>
          </div>
        </Reveal>
      </section>
      ...
      {/* ERAS */}
      {!anyVisible && (
        <section style={{ padding: "120px 32px", borderTop: "1px solid var(--ink)", textAlign: "center" }}>
          <div className="mono-cap" style={{ marginBottom: 16 }}>
            <T en="No projects match" zh="没有匹配的项目" />
          </div>
          <p style={{ fontFamily: "var(--font-serif)", fontStyle: "italic", fontSize: "clamp(20px,2.4vw,32px)", color: "var(--ink-mid)" }}>
            <T en="Try another keyword or tag." zh="换一个关键词或标签试试。" />
          </p>
          <button className="btn btn-ghost" style={{ marginTop: 28 }} onClick={() => { setFilter("All"); setQuery(""); }}>
            <T en="Reset filters" zh="重置筛选" />
          </button>
        </section>
      )}
      {filteredEras.map(({ era, visible, projects }, eIdx) => {
        ...
        <section key={era.years} style={{ padding: "96px 32px", borderTop: "1px solid var(--ink)", background: eIdx % 2 === 1 ? "var(--mist)" : "var(--paper)" }}>
          <div className="grid-12" style={{ alignItems: "start", gap: 32 }}>
            ...
            <div style={{ gridColumn: "6 / -1" }}>
              {era.archive ? (
                <Reveal>
                  <div style={{ position: "relative", color: "var(--paper)", overflow: "hidden", minHeight: 520 }}>
                    {/* Background image */}
                    <img
                      src="images/projects/archive-21-23/hero.jpg"
                      alt=""
                ...
                <div style={{ display: "grid", gridTemplateColumns: projects.length >= 3 ? "repeat(3, 1fr)" : "repeat(2, 1fr)", gap: 24 }}>
                  {projects.map((p, i) => {
                    const card = {
                      title: p.title, titleEm: p.titleEm, year: p.year,
                      role: t(p.role_en, p.role_zh), img: t(p.img_en, p.img_zh),
                      tone: p.tone, tags: p.tags, image: p.image, position: p.position,
                    };
                    return (
